...
export MYSQL_REPLICATION_PASSWORD="${MYSQL_REPLICATION_PASSWORD:-Admin123}"
export MYSQL_APP_PASSWORD="${MYSQL_APP_PASSWORD:-Admin123}"
export MYSQL_CONFIGURE_PROXY=true
compose=(docker compose -f "$script_dir/docker-compose-replication.yaml")
up=("${compose[@]}")
up+=(--profile proxy)
manage=("${compose[@]}")
manage+=(--profile proxy)
...
case "$action" in
  ...
  clean)
    ...
    printf '将删除此 Compose 项目的容器和数据卷：%s\n' "$script_dir/docker-compose-replication.yaml" >&2
    "${manage[@]}" down --volumes ;;
  ps) "${manage[@]}" ps "$@" ;;
