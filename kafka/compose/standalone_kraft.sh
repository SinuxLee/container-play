...
export KAFKA_EXTERNAL_HOST="${KAFKA_EXTERNAL_HOST:-$HOST_BIND_ADDRESS}"
compose=(docker compose -f "$script_dir/docker-compose-kraft.yaml")
up=("${compose[@]}")
manage=("${compose[@]}")
...
case "$action" in
  up)
    if [[ "$KAFKA_EXTERNAL_HOST" == 0.0.0.0 || "$KAFKA_EXTERNAL_HOST" == "::" ]]; then
      echo "KAFKA_EXTERNAL_HOST must be an address clients can reach" >&2
      exit 2
    fi
    "${up[@]}" up -d "$@" ;;
  down) "${manage[@]}" down "$@" ;;
  clean)
    ...
    printf '将删除此 Compose 项目的容器和数据卷：%s\n' "$script_dir/docker-compose-kraft.yaml" >&2
    "${manage[@]}" down --volumes ;;
  ps) "${manage[@]}" ps "$@" ;;
