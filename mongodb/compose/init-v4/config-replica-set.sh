...
mongo --host mongodb-cfg:27019 --quiet --eval '
  var status;
  ...
  if (status.ok !== 1) {
    ...
    var result = rs.initiate({ _id: "cfgReplSet", configsvr: true, members: [{ _id: 0, host: "mongodb-cfg:27019" }] });
    if (result.ok !== 1) throw new Error("rs.initiate failed");
  }
'

for ((attempt = 1; attempt <= 60; attempt++)); do
  if mongo --host mongodb-cfg:27019 --quiet --eval 'if (!db.isMaster().ismaster) quit(1)' >/dev/null 2>&1; then
    exit 0
  fi
  sleep 2
done
