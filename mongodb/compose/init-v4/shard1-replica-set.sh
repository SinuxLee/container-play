...
mongo --host mongodb-shard1:27018 --quiet --eval '
  var status;
  ...
  if (status.ok !== 1) {
    ...
    var result = rs.initiate({ _id: "shard1ReplSet", members: [{ _id: 0, host: "mongodb-shard1:27018" }] });
    if (result.ok !== 1) throw new Error("rs.initiate failed");
  }
'

for ((attempt = 1; attempt <= 60; attempt++)); do
  if mongo --host mongodb-shard1:27018 --quiet --eval 'if (!db.isMaster().ismaster) quit(1)' >/dev/null 2>&1; then
    exit 0
  fi
  sleep 2
done
